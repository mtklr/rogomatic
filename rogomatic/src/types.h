...
it also it keeps the fan from spinning up (saving battery life, reducing energy
use in general or reducing fan noise).

at higher levels it goes even slower because it should be more interesting (or
at least in theory...)

for your specific machine you might have to adjust this up or down to suit your
own preferences.
...
#define direc(r,c)	\
	(r>0?(c>0?7:(c<0?5:6)):(r<0?(c>0?1:(c<0?3:2)):(c>0?0:4)))
